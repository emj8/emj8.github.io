...
### Data Acquisition
The data of the restaurants was acquired from [Yelp.com](https://www.yelp.com/search?find_desc=fine+dining&find_loc=New+York%2C+NY) using web scraping techniques with Selenium tool in Python and FireFox WebDriver. <p style='text-align: justify; display:inline'>About a thousand restaurants data was collected by taking a list from each price range category (Inexpensive $, Moderate $$, Pricey $$$, Ultra High-End $$$$).
</p>
<p style='text-align: justify'>
From each restaurant, thirteen features were collected that include: Restaurant Name, Rating, Number of Reviews, Price Range, Food Type, Number of Photos, and the Opening Hours for each day of the week (from Monday to Sunday). The figure below shows how the data frame look like.
</p>
